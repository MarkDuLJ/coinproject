...
export default function StreamRateIndicator() {
  ...
  */
  const calculateStreamRate = async () => {
    ...
    */
    let aptPerSec = 0;
    // await getSenderStreams()
    await getReceiverStreams()
   ...
   */
    ...
    */
    const response = await fetch (
      `https://fullnode.testnet.aptoslabs.com/v1/accounts/${process.env.RESOURCE_ACCOUNT_ADDRESS}/events/${process.env.MODULE_ADDRESS}::${process.env.MODULE_NAME}::ModuleEventStore/stream_create_events?limit=10`,
      {
        method: 'GET'
      }
    );
    ...
    const formattedData=eventData.map(
      (e:any)=>({
        amount:parseFloat(e.data.amount)/100000000,
        time:new Date(e.data.timestamp*1000).toISOString()
      }));

    // console.log("SENDER_STREAM:",formattedData);
    /* 
  ...
  const getReceiverStreams = async () => {
    ...
    */
            const body = {
              // function:"0x1::coin::get_receivers_streams",
              function:`${process.env.RESOURCE_ACCOUNT_ADDRESS}::pay_me_a_river::get_receivers_streams`,
    ...
    */
        ...
        console.log("STREAM_ACCEPT:",eventData);
